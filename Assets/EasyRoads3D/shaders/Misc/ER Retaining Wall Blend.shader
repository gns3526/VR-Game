// © 2021 EasyRoads3D
// This shader blends two textures with the option to set the Blend Threshold between the two textures. It can be used for example for retaining walls. In this case the first texture is the texture at the road side, the second texture is the actual wall texture. 
	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 )
			{
				...
				float3 unpack5 = UnpackNormalScale( tex2D( _BumpMap, texCoord39 ), _BumpScale1 );
				unpack5.z = lerp( 1, unpack5.z, saturate(_BumpScale1) );
				float2 texCoord37 = IN.ase_texcoord5.xy * float2( 1,1 ) + float2( 0,0 );
				float3 unpack23 = UnpackNormalScale( tex2D( _BumpMap2, texCoord37 ), _DetailNormalMapScale1 );
				unpack23.z = lerp( 1, unpack23.z, saturate(_DetailNormalMapScale1) );
				float temp_output_22_0 = ( IN.ase_color.a / _Threshold );
				float3 lerpResult24 = lerp( unpack5 , unpack23 , temp_output_22_0);
